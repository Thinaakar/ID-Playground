#!/usr/bin/env python3
"""Generate the MonstarX Indonesia MCP playground HTML from tool metadata.

Run from anywhere:  python build/build.py
Reads:  build/data.min.json  (tool schemas + captured example responses)
Writes: public/index.html  and  indonesia-mcp-playground.html  (identical)
"""
import os
...
:root{
  ...
  --dot-catalog:#64748b;--dot-weather:#16a34a;--dot-hazards:#dc2626;--dot-geo:#0891b2;--dot-civic:#7c3aed;--dot-places:#db2777;--dot-finance:#ea580c;--dot-transport:#2563eb;--dot-news:#0ea5e9;--dot-health:#059669;--dot-culture:#d97706;
}
...
<div class="topbar">
  ...
  <div class="endpoint-pill" id="epPill" title="Copy MCP endpoint">
    ...
    <span class="u">id-mcp-staging.monstarxapp.com/mcp</span>
  </div>
  ...
  <div class="lang-wrap">
    ...
    <div class="lang-menu" id="langMenu" role="listbox">
      ...
      <button type="button" data-lang="id" role="option">🇮🇩 Bahasa Indonesia</button>
    </div>
  </div>
</div>
<div class="layout" id="top">
  ...
  <aside class="sidebar" id="sidebar">
    <div class="search"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="m21 21-4.3-4.3"/></svg>
      <input id="filter" type="text" data-i18n-placeholder="filterTools" placeholder="Filter 55 tools…" autocomplete="off" spellcheck="false"></div>
  ...
  <main class="content">
    <section class="hero" id="overview">
      ...
      <div class="stats">
        <div class="stat"><div class="n">55</div><div class="l" data-i18n="statTools">ready-made tools</div></div>
        <div class="stat"><div class="n">25</div><div class="l" data-i18n="statSources">free public sources</div></div>
    ...
    <section id="playground">
      ...
      <p class="sec-blurb" data-i18n-html="secPlaygroundBlurb">Pick a tool, tweak the inputs, hit <b>Run</b> — the query goes straight to the live MCP server and streams back real Indonesia data. No sign-up, nothing to install. Try a one-click example to start:</p>
      <div class="pg">
        <div class="pg-note" id="pgNote"></div>
        <div class="pg-top">
          <span class="lbl" data-i18n="tryLabel">Try</span>
          <button class="chip-ex" data-ex="wx24" data-i18n-chip="chipWx"><span class="e">⛅</span> Jakarta weather 24h</button>
    ...
    <section id="quickstart">
      ...
      <p class="sec-blurb" data-i18n="secConnectBlurb">MonstarX Indonesia MCP is a remote HTTP server speaking the Model Context Protocol. Point any MCP-capable client at the endpoint — no auth handshake, it's stateless.</p>
      <div class="grid2">
    ...
    <section id="response">
      ...
      <div class="card" style="margin-top:16px"><table class="ptable" style="margin:0"><thead><tr><th data-i18n="thField">Field</th><th data-i18n="thMeaning">Meaning</th></tr></thead><tbody>
        <tr><td class="pn">source</td><td data-i18n="fSource">Upstream platform — BMKG, Open-Meteo, Nominatim, HDX, Comuline, Nager.Date, etc.</td></tr>
        <tr><td class="pn">agency</td><td data-i18n="fAgency">Originating body — BMKG, Bank Indonesia, KPU, Kemenag, OJK, …</td></tr>
        ...
        <tr><td class="pn">retrieved_at</td><td data-i18n="fRetrieved">Server fetch time (UTC). Live timestamps inside payloads are often WIB (+07:00).</td></tr>
    ...
    <section id="reference">
      <h2><span class="num">05</span><span data-i18n="secTools">All 55 tools</span></h2>
      <p class="sec-blurb" data-i18n-html="secToolsBlurb">Every tool is prefixed <code>id_</code>; required params are marked <span style="color:var(--accent)">*</span>. Hit <b>Try in playground</b> on any tool to load it above with a working example.</p>
      <div id="tools"></div>
    </section>
    ...
    <footer>
      ...
      <div class="cols">
        ...
        <div><h4 data-i18n="ftServer">Server</h4><div>MonstarX Indonesia MCP · v0.1.0</div><div>Protocol <code>2025-06-18</code> · <span class="tag">staging</span></div><div><a href="https://monstarx.com">monstarx.com</a></div></div>
      </div>
...
const EP='https://id-mcp-staging.monstarxapp.com';
function liveBase(){
  ...
  // Local npm start: same-origin /mcp is proxied by server.js
  if(/^(localhost|127\.0\.0\.1)$/i.test(location.hostname))return '';
  // Cloudflare Pages / static hosts have no /mcp proxy — call staging directly (CORS open)
  return EP;
}
let LANG=localStorage.getItem('mx-lang-id')||'en';
...
filterTools:'Filter 55 tools…',eyebrow:'MonstarX · Indonesia MCP',
...
secPlaygroundBlurb:'Pick a tool, tweak the inputs, hit <b>Run</b> — the query goes straight to the live MCP server and streams back real Indonesia data. No sign-up, nothing to install. Try a one-click example to start:',
tryLabel:'Try',
chipWx:'Jakarta weather 24h',chipQuake:'Recent quakes',chipGeo:'Geocode Monas',chipPostal:'Postal 10110',
...
useWxT:'Weather-aware apps',useWxP:'Adm4 village codes, BMKG overviews, 24h/4-day forecasts, UV, rain, and air quality for Jakarta or any Indonesian village.',
useDisT:'Disaster awareness',useDisP:'Surface recent earthquakes, tsunami potential, Jakarta floods, volcanoes, and nearby evacuation points.',
...
useDataT:'Open data explorer',useDataP:'Search HDX Indonesia packages, inspect metadata, and query datastore tables — plus live news headlines.',
...
secTools:'All 55 tools',
secToolsBlurb:'Every tool is prefixed <code>id_</code>; required params are marked <span style="color:var(--accent)">*</span>. Hit <b>Try in playground</b> on any tool to load it above with a working example.',
...
contacting:'contacting server…',hintCors:'Could not reach the MCP server. On this Pages site Live Run calls staging directly. Locally, run ID-MCP (`npm run dev` in Backend/ID_mcp on :8787) and the playground (`npm start`) so /mcp can proxy.',
...
contacting:'menghubungi server…',hintCors:'Tidak bisa menjangkau server MCP. Di situs Pages, Run memanggil staging langsung. Lokal: jalankan ID-MCP (`npm run dev` di Backend/ID_mcp :8787) dan playground (`npm start`) agar /mcp bisa memproksi.',
...
function applyLang(lang){
  ...
  localStorage.setItem('mx-lang-id',lang);
...
content.innerHTML=content.innerHTML
 .replace('__CB_CC__',codeblock('shell',sh('claude mcp add --transport http \\\n  id-mcp '+EP+'/mcp'),{raw:'claude mcp add --transport http id-mcp '+EP+'/mcp'}))
 .replace('__CB_CURSOR__',codeblock('mcp.json',hl({mcpServers:{indonesia:{type:"http",url:EP+'/mcp'}}})))
 .replace('__CB_DESK__',codeblock('json',hl({mcpServers:{indonesia:{command:"npx",args:["-y","mcp-remote",EP+'/mcp']}}})))
 .replace('__CB_QS__',codeblock('bash',sh(curlFor('id_weather_24h',{area_code:'31.71.03.1001'})),{raw:curlFor('id_weather_24h',{area_code:'31.71.03.1001'})}))
 .replace('__CB_ENV__',codeblock('structuredContent — id_weather_24h',hl({source:"Open-Meteo Forecast",agency:"Open-Meteo",retrieved_at:"2026-08-26T04:00:00.000Z",api:"v1/forecast (hourly forecast 24h)",area_code:"31.71.03.1001",location:{latitude:-6.175392,longitude:106.827153},data:{hourly:{time:["2026-08-26T00:00"],temperature_2m:[26.4]}}})))
 .replace('__CB_ERR__',codeblock('isError: true — missing required argument',hl({content:[{type:"text",text:"MCP error -32602: Input validation error: Invalid arguments for tool id_postal_code: [{ path: ['zipcode'], message: 'expected string, received undefined' }]"}],isError:true})));
...
function timeOnly(iso){try{return new Date(iso).toLocaleString('en-ID',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'});}catch(e){return iso||'';}}
function wkday(iso){try{return new Date(iso+(iso&&iso.length===10?'T00:00:00+07:00':'')).toLocaleDateString('en-ID',{weekday:'short',day:'numeric',month:'short'});}catch(e){return iso;}}
...
/* Indonesia map bounds (Sumatra to Papua) */
...
function overviewText(p){const d=p.data||{};const text=d.text||d.headlineText||(typeof d==='string'?d:JSON.stringify(d).slice(0,400));const title=d.targetArea||d.headTitle||p.area_code||'overview';return '<div class="vhead">BMKG overview · '+esc(title)+' · '+timeOnly(d.reportDatetime||'')+'</div><div class="ccard"><div style="white-space:pre-wrap;line-height:1.55">'+esc(String(text).slice(0,1200))+'</div></div>';}
function officesView(p){const o=p.offices||p.areas||[];return '<div class="vhead">'+num(p.total_offices||p.total_areas||o.length)+' BMKG starter areas</div><div class="vtwrap"><table class="vt"><thead><tr><th>Code</th><th>Name</th><th>English</th></tr></thead><tbody>'+o.slice(0,40).map(x=>'<tr><td class="mono"><b>'+esc(x.area_code)+'</b></td><td>'+esc(x.name)+'</td><td>'+esc(x.en_name||x.city||x.province||'')+'</td></tr>').join('')+'</tbody></table></div>'+(o.length>40?'<div class="vnote">Showing 40 of '+o.length+'.</div>':'');}
...
function bojView(p){
  ...
  return '<div class="vhead">BI · '+esc(p.db||'')+(series?' · '+head:'')+'</div>'+(rows.length?genTable(rows.slice(0,40)):codeblock('data',hl(d),{scroll:true}));
}
...
const root=document.documentElement,saved=localStorage.getItem('mx-theme-id');if(saved)root.setAttribute('data-theme',saved);
document.getElementById('themeBtn').onclick=()=>{const cur=root.getAttribute('data-theme')||(matchMedia('(prefers-color-scheme:dark)').matches?'dark':'light');const nx=cur==='dark'?'light':'dark';root.setAttribute('data-theme',nx);localStorage.setItem('mx-theme-id',nx);};
...
standalone=('<!doctype html><html lang="en"><head><meta charset="utf-8">'
 ...
 '<title>MonstarX Indonesia MCP | Live</title>'
 +FAVICON_LINK+
 '<meta name="description" content="Indonesia government and public open data as 55 MCP tools any AI agent can call. Live in-browser playground: weather, earthquakes, geocoding, KRL, news, holidays, open data.">'
 +STYLE+'</head><body>'+BODYTOP+SCRIPT.replace('__DATA__',DATA)+'</body></html>')
os.makedirs(os.path.join(REPO,'public'),exist_ok=True)
open(os.path.join(REPO,'indonesia-mcp-playground.html'),'w',encoding='utf-8').write(standalone)
...
print('wrote public/index.html and indonesia-mcp-playground.html ('+str(len(standalone))+' bytes)')
